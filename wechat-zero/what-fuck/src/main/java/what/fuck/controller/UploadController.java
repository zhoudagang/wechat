...
@Controller
public class UploadController extends AbstractBaseResource {
	...
	public String show(String groupId, String fileId, Model model) throws Exception {
		...
		ClientGlobal.init(classPathResource.getClassLoader().getResource("fastdfs_client.conf").getPath());
		...
		if (request instanceof MultipartHttpServletRequest) { // 如果你现在是MultipartHttpServletRequest对象
			...
			while (iter.hasNext()) {
				...
				if (photo != null) { // 现在有文件上传
					...
					String fileExtName = photo.getContentType().substring(photo.getContentType().lastIndexOf("/") + 1);
					// 通过ClassPath路径获取要使用的配置文件
					ClassPathResource classPathResource = new ClassPathResource("fastdfs_client.conf");
					// 进行客户端访问的整体配置，需要知道配置文件的完整路径
					ClientGlobal.init(classPathResource.getClassLoader().getResource("fastdfs_client.conf").getPath());
					...
					String upload_file = storageClient.upload_file1(photo.getBytes(), fileExtName, metaList);
